...
pub struct State {
    ...
    clients: Arc<Mutex<HashMap<String, Vec<SocketAddr>>>>,
...
impl State {
    ...
    pub async fn start_streaming_video(&self, video_path: String) -> anyhow::Result<JoinHandle<anyhow::Result<()>>> {
        ...
        self.clients.lock().unwrap().insert(video_path.clone(), Vec::new());

        let state = self.clone();

        Ok(tokio::spawn(async move {
            loop {
                if let Ok(video_bytes) = video_process.recv().await {
                    let clients_list = state.clients.lock().unwrap().get(&video_path).cloned().unwrap_or_default();
                    let packet = SCPacket::VideoPacket(video_bytes);
    ...
    loop {
        ...
        match packet {
            ...
            CSPacket::RequestVideo(video_path) => {
                ...
                state.clients.lock().unwrap()
                    .entry(video_path)
                    .or_insert_with(Vec::new)
                    .push(addr);
            }
            CSPacket::StopVideo(video_path) => {
                if let Some(subscribers) = state.clients.lock().unwrap().get_mut(&video_path) {
